...
found:
  ...
  //Create a kernel page table and allocate a kernel stack
...
freeproc(struct proc *p)
{
  ...
  if(p->kernelPageTable)
    proc_free_kernel_pagetable(p->kernelPageTable);
  p->kernelPageTable=0;
  if(p->pagetable)
...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  kvmcopymappings(p->pagetable, p->kernelPageTable, 0, p->sz); // 同步程序内存映射到进程内核页表中
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    // 内核页表中的映射同步扩大
    if(kvmcopymappings(p->pagetable, p->kernelPageTable, sz, n) != 0) {
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0||
     kvmcopymappings(np->pagetable, np->kernelPageTable, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
